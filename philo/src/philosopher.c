...
int		had_a_meal(t_data *data, t_phi *me)
{
	...
	print_action(&data->phi[me->id], me->id, EAT, 0);
...
int		had_a_nap(t_data *data, t_phi *me)
{
	...
	print_action(&data->phi[me->id], me->id, SLEEP, 0);
	diff = get_current_time_ms() - me->last_meal - data->c->time_to_die;
	if (diff >= 0) {
		usleep_ms(diff);
		me->status = DEAD;
...
void	set_final_status(t_data *data, t_phi *me)
{
	if (is_dead(data, me)) {
		me->status = DEAD;
		print_action(&data->phi[me->id], me->id, DEAD, 0);
		pthread_mutex_lock(data->dead_mutex);
		data->mon->dead_num++;
		pthread_mutex_unlock(data->dead_mutex);	
	} else {
		me->status = DONE;
		print_action(&data->phi[me->id], me->id, DONE, 0);
	}
...
void	*philosopher(void *data_pointer)
{
	...
	while (me->must_eat_times && me->status != DEAD)
	{
		...
		*/
		...
		print_action(&data->phi[me->id], me->id, THINK, 0);
		me->status = THINK;
	}
