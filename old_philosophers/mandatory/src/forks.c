...
int	is_forks_taken(t_p *args, int first_fork, int second_fork)
{
	...
	printf("%d id | %lld curr\n", args->id, get_current_time_ms());
	...
	if (!first_result)
	{
		...
		if (!second_result)
		{
			print_action(args, get_current_time_ms(), TAKE_FORK, first_fork);
			print_action(args, get_current_time_ms(), TAKE_FORK, second_fork);
			return (1);
		}
		else
